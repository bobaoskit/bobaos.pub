...
let BobaosSdk = params => {
  ...
  _bobaos.on("open", async _ => {
    // remove listeners for indications
    _bobaos.removeAllListeners("DatapointValue.Ind");
    _bobaos.removeAllListeners("ServerItem.Ind");

    // now, load items, datapoints
    ...
    // return indications on baos to true,
  ...
  self.getDescriptionString = async id => {
    if (typeof id === "number") {
      if (id < 1 || id > datapoints.MAX_DATAPOINTS_NUM) {
        throw new RangeError(`Datapoint id should be in range ${1}-${datapoints.MAX_DATAPOINTS_NUM}.`);
      }
      return await _bobaos.getDescriptionString(id);
    }

    throw new TypeError("Payload should be datapoint id");
  };

  self.setValue = async (payload, command = "set and send") => {
    if (Array.isArray(payload)) {
      payload.forEach(t => {
      ...
      let headerSize = 6;
      let maxReqLen = bufferSizeUInt - headerSize;
      // default 250 - 6 = 244

      // for test purposes restrict max len
      // maxReqLen = 15;

      // array of arrays with multiple request payload
      // in most cases it will contain just one array
      //
      // [[{id: id1, value: v1}, {id: id2, value, v2}...], [...]]
      let requests = [];
      ...
      while (i < payload.length) {
        let t = payload[i];
        let descr = datapoints.getStoredDescription(t.id);
        let itemLength = descr.length;
        // header for one item follows:
        // first dp: id, command, length, value
        // [2, 1, 1, 1-14]
        reqLen += 4;
        reqLen += itemLength;
        if (reqLen <= maxReqLen) {
          ...
          if (i === payload.length - 1) {
            requests.push(currentReqs);
          }
          i += 1;
        } else {
          // almost impossible case, but to prevent infinite loop
          if (currentReqs.length === 0) {
            let err = `setValue request buffer too big for one datapoint. `;
            err += `Id: ${t.id}. Maximum: ${maxReqLen}, calculated: ${reqLen}`;
            throw new Error(err);
          }

          // push to requests array
          requests.push(currentReqs);
          currentReqs = [];
          reqLen = 0;
        }
      ...
      results = await Promise.all(
        requests.map(async r => {
          ...
          await _bobaos.setMultipleValues(rawValues, command);

          // if baos accepted raw values, then all good
          // converting depends on "knx-dpts-baos" package
          await datapoints.processRawValues(rawValues);

          return self.getStoredValue(r.map(v => v.id));
        })
      );

      // now transform to array without arrays inside
      let results2return = [];
      results.forEach(r => {
        results2return = results2return.concat(r);
      });
    ...
    if (Array.isArray(id)) {
      ...
      while (idUniq.length > 0) {
        ...
        while (true) {
          ...
          let item = datapoints.getStoredDescription(current);
      ...
      // process all values that was get from BAOS module
      // GetDatapointValue.Req(start, number) returns all datapoint
      // values within range start-num. Even if datapoint does not exist.
      // process all values from response
      // so, no inforation wasted, datapoint values stored
      let result = await datapoints.processRawValues(valuesFlat);

      // but return in result only requested ids.
      result = result.filter(filterIds);
  ...
  self.getStoredValue = async id => {
    ...
    if (Array.isArray(id)) {
      ...
      await Promise.all(
        id.map(async d => {
          let datapoint = datapoints.getStoredDescription(d);
          if (datapoint) {
  ...
  self.readValue = async id => {
    ...
    if (Array.isArray(id)) {
      id.forEach(t => {
        ...
        if (id < 1 || id > datapoints.MAX_DATAPOINTS_NUM) {
          let m = datapoints.MAX_DATAPOINTS_NUM;
          throw new RangeError(`Datapoint id should be in range ${1}-${m}.`);
        }
      });

      // basically, it is the same SetDatapointValue.Req,
      // the lengths of request are the same
      ...
      // in most cases it will contain just one array
      //
      // [[{id: id1, length: l1}, {id: id2, length: l2}...], [...]]
      let requests = [];
      ...
      while (i < id.length) {
        let t = id[i];
        let descr = datapoints.getStoredDescription(t);
        let itemLength = descr.length;
        // header for one item follows:
        // first dp: id, command, length, value
        // [2, 1, 1, 1-14]
        ...
        reqLen += itemLength;
        if (reqLen <= maxReqLen) {
          currentReqs.push({ id: t, length: itemLength });
          ...
          // almost impossible case, but to prevent infinite loop
          if (currentReqs.length === 0) {
            let err = `readValue request buffer too big for one datapoint. `;
            err += `Id: ${t.id}. Maximum: ${maxReqLen}, calculated: ${reqLen}`;
            throw new Error(err);
          }
      ...
      await Promise.all(
        requests.map(async r => {
          // set multiple values
          await _bobaos.readMultipleDatapoints(r);
        })
      );

      // if everything went ok then good
      return true;
    }
