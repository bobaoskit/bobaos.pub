...
let BobaosSdk = params => {
  ...
  self.getValue = async id => {
    ...
    if (Array.isArray(id)) {
      id.forEach((item, index) => {
        if (typeof item !== "number") {
          throw new RangeError(`Wrong datapoint id in array. Item: ${item}; index: ${index}.`);
        }
      });
      id.sort((a, b) => {
      ...
      while (i < imax) {
        ...
        if (isNaN(start)) {
          throw new RangeError(`Cannot get datapoint id from array at index ${i}`);
        }
    ...
    throw new RangeError(`Wrong datapoint id type. Should be number or array of numbers.`);
  };
  ...
  self.getStoredValue = async id => {
    ...
    if (Array.isArray(id)) {
      ...
      id.forEach(t => {
        if (typeof t !== "number") {
          throw new Error("Wrong datapoint id type. Should be number or array of numbers.");
        }
        if (id < 1 || id > datapoints.MAX_DATAPOINTS_NUM) {
          throw new RangeError(`Datapoint id should be in range ${1}-${datapoints.MAX_DATAPOINTS_NUM}`);
        }
    ...
    if (Array.isArray(id)) {
      let ids = await Promise.all(
        id.map(async (i, index) => {
          ...
          throw new RangeError(`Wrong datapoint id: "${i}" at index: ${index}`);
        })
      );
