...
let BobaosSdk = params => {
  ...
  _bobaos.on("ServerItem.Ind", payload => {
    payload.forEach(t => {
      let serverItemIndex = SI.getIndexById(t.id);
      let serverItem = SI.process(t);
      SI.update(serverItemIndex, serverItem);
      self.emit("server item", serverItem);
    });
  });
  ...
  self._loadDatapoints = async _ => {
    ...
    await _bobaos.setServerItem(SI.ServerItems.IndicationSending, Buffer.alloc(1, 0x00));
    let buff_size = (await _bobaos.getServerItem(SI.ServerItems.CurrentBufferSize, 1))[0].value.readUInt16BE(0);
    ...
    await _bobaos.setServerItem(SI.ServerItems.IndicationSending, Buffer.alloc(1, 0x01));
    // TODO: get bus connected state
    if (SI.getValueByName("BusConnectionState")) {
  ...
  self._loadServerItems = async (start = 1, number = 17) => {
    ...
    payload.forEach(t => SI.push(SI.process(t)));
  ...
  self.getServerItemNames = _ => {
    return SI.getNames();
  };

  self.getServerItem = payload => {
    if (payload === null) {
      return SI.getAllValues();
    }
    if (Array.isArray(payload)) {
      return payload.map(SI.findByName);
    }
    if (typeof payload === "string") {
      return SI.findByName(payload);
    }

    return SI.findById(payload);
  };

  // programming mode
  self.setProgrammingMode = async value => {
    let id = SI.ServerItems.ProgrammingMode;
    ...
    let serverItemIndex = SI.getIndexById(s.id);
    let serverItem = SI.process(s);
    SI.update(serverItemIndex, serverItem);

    return self.getServerItem("ProgrammingMode");
  };
  ...
  self.getDescription = async id => {
    if (id === null) {
      return await datapoints.getAllDescriptions().filter(t => t);
    }
    if (Array.isArray(id)) {
      return id.map(async i => {
  ...
  self.setValue = async payload => {
    if (Array.isArray(payload)) {
      // set multiple values
      let rawValues = payload.map(async v => {
        let {id, value} = v;
