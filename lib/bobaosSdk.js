...
let BobaosSdk = params => {
  ...
  self.getValue = async id => {
    ...
    if (Array.isArray(id)) {
      ...
      console.log("buff_size", bufferSizeUInt);

      // default 250 - 6 = 244
      console.log("max_data_len", maxResLen);
      ...
      while (i < imax) {
        ...
        while (j < jmax) {
          let current = start + j;
          let item = await self.getDescription(current);
  ...
  self.pollValues = async _ => {
    let descriptions = await self.getDescription(null);
