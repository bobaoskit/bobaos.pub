...
let BobaosSdk = params => {
  ...
  self._loadDatapoints = async _ => {
    ...
    for (let i = 0; i < datapoints.MAX_DATAPOINTS_NUM; i += number) {
      ...
      const prom = _bobaos.getDatapointDescription(i + 1, number).then(chunk => {
    ...
    // let dps = await Promise.all(getDatapointDescrPromises.map(p => p.catch(e => {
    //   console.log(`Error getting datapoint description: ${e.message}. Probably not configured.`);
    // })));
  ...
  self._loadServerItems = async (start = 1, number = 17) => {
    ...
    await Promise.all(payload.map(async t => {
      let {id, value} = t;
  ...
  self.setValue = async payload => {
    if (Array.isArray(payload)) {
      payload.forEach(t => {
        ...
        if (!itemHasValue) {
          throw new Error(`Value item should have "value" field.`);
        }
        let {id, value} = t;
        if (typeof id !== "number") {
    ...
    if (!payloadHasId) {
      throw new TypeError(`Payload object should have "id" field.`);
    }
    if (!payloadHasValue) {
      throw new TypeError(`Payload object should have "value" field.`);
    }
  ...
  self.getValue = async id => {
    ...
    if (Array.isArray(id)) {
      ...
      while (i < imax) {
        ...
        while (j < jmax) {
          ...
          if (currentResLen + currentItemResLen < maxResLen) {
            ...
            // proceed next. if current datapoint presents in array, then
            let index = idUniq.findIndex(t => parseInt(t) === current);
            if (index > -1) {
              // start next `i` loop from that index
              i = index;
            } else {
              // no datapoint in origin array, just proceed to next item
              i += 1;
            }
            break;
          }
          ...
          // if current datapoint is in array, then assign i to this index
          // so, we know place in id array
          let index = idUniq.findIndex(t => parseInt(t) === current);
          if (index > -1) {
            i = index;
          }
      ...
      console.log(`get value will send ${JSON.stringify(map)} requests`);
  ...
  self.pollValues = async _ => {
    // TODO: get valid datapoints then return self.getValue([...])
