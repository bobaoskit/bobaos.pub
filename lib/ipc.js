...
let IPC = params => {
      ...
      if (_timeSyncParams.sync_enabled) {
        if (_timeSyncParams.check_enabled) {
          ...
          return redisClient.get(_timeSyncParams.redis_key, (err, res) => {
            ...
            queueToProcess._throwLatestJob = _ => {
              done(new Error("Throw latest job on reset"));
              queueToProcess._throwLatestJob = null;
            };
            return _emitReqRes();
          });
        }
  ...
  self.reset = async _ => {
    if (typeof reqQ._throwLatestJob === "function") {
      // throw an erro to latest job, so queue will be free
      reqQ._throwLatestJob();
    }
  };
  reqQ.on("ready", _ => {
    if (_timeSyncParams.sync_enabled) {
